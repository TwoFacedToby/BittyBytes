...
public class HighlightLogic {
                ...
                if (newPiece.getType().equals("Empty")) {
                    ...
                    if (newPiece.getMarked()) {
                        ...
                        if (newPiece.getType().equals("King")) {
                            running = false;
                            ChessHandler.get().getViewController().winner(!newPiece.getColor());
                        }
                        ChessHandler.get().getViewController().killPiece(newPiece);
    ...
    private void moveTo(Piece newPiece){
        ...
        if(ChessHandler.get().getCheckLogic().noChecksForSamePayerAt(moveTo, current)){
            if(current.getType().equals("Pawn")){
                if(current.getColor()) //Black
                {
            ...
            ChessHandler.get().getViewController().movePiece(current, moveTo);
            current.setPosition(moveTo);
            Pieces.getInstance().setTurn(!Pieces.getInstance().isTurn());
            ChessHandler.get().getViewController().highlightTurn();
            resetBoard();
        }
