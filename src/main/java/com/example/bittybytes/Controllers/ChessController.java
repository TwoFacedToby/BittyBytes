...
import com.example.bittybytes.Chess.logic.Images;
...
public class ChessController {
    ...
    ArrayList<Piece> pieces = Pieces.getInstance().getPieces();

    public void initialize() {
        initGrid();
        initPieces();
        highlightTurn();
        reset.setOnAction(e -> resetPieces());
    }
    ...
    private void resetPieces(){
        System.out.println("Reset : " + pieces.size());
        pieces.clear();
        Pieces.getInstance().reset();
        pieces = Pieces.getInstance().getPieces();
        for(int i = 0; i < pieceImageViews.size(); i++){
            if(dead1.getChildren().contains(pieceImageViews.get(i))){
                dead1.getChildren().remove(pieceImageViews.get(i));
            }
            else if(dead2.getChildren().contains(pieceImageViews.get(i))){
                dead2.getChildren().remove(pieceImageViews.get(i));
            }else{
                for(int n = 0; n < fields.size(); n++){
                    if(fields.get(n).getChildren().contains(pieceImageViews.get(i))){
                        fields.get(n).getChildren().remove(pieceImageViews.get(i));
                    }
                }
            }
        }
        ...
        initPieces();
        Pieces.getInstance().setTurn(false);
        ChessHandler.get().setRunning(true);
        highlightTurn();
    ...
    public void killPiece(Piece piece) {
        ...
        for (int i = 0; i < pieces.size(); i++) {
            if (pieces.get(i) == piece) {
                ...
                dead1.setSpacing(1);
                dead2.setSpacing(1);
                pieceImageViews.get(i).setFitWidth(40);
                pieceImageViews.get(i).setFitHeight(40);
    ...
    public void fieldClicked(int index) {
        ...
        ChessHandler.get().choosePiece(piece);
        updateBoard();
    }
    ...
    private void mouseOver(int index, boolean isOver) {
        ...
        if (ChessHandler.get().isHighlighted(index)) { //Highlight
